...
def generate_report_with_llm(student_ability, job_profile, job_name, path_suggestions):
    """
    【修复完整版】生成 5 个完整章节，绝不中途截断！
    1.自我认知 2.人岗匹配 3.职业发展路径 4.分阶段行动计划 5.评估与调整
    """
    context = {
        '学生信息': {
    ...
    # ✅ 修复重点：强制要求输出 5 个完整章节，绝不提前结束
    prompt = f"""你是专业职业规划师。
请根据学生数据生成【完整】职业生涯发展报告，必须包含以下 5 个部分，**缺一不可**：

一、自我认知总结
二、人岗匹配分析（优势、差距）
三、职业发展路径（晋升、转岗）
四、分阶段行动计划（短期：1年内；中期：1-3年；长期：3-5年）
五、评估与调整建议（评估周期、调整指标、动态优化方法）

要求：
- 内容必须具体、个性化，基于真实数据
- 不要使用通用模板
- 必须完整输出 5 部分，**绝对不能中途结束**
- 使用 Markdown 格式
...
"""
    ...
    try:
        result = call_llm(prompt, temperature=0.7, max_tokens=4000)
...
@token_required
def get_career_report(user, report_id):
...
@token_required
def get_interest_report(user, result_id):
...
@token_required
def get_match_report(user, match_id):
...
@token_required
def update_career_report(user, report_id):
